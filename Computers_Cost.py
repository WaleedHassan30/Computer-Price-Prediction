...
st.success("🎨Data Science Project | How Much Do Computers Cost?")
...
if selection_type == "Desktop":
    ...
    if submit:
        result_from  = round(predict-1000)
        result_to = round(predict+1000)
        st.subheader(f"Predicted price for this Computer could be between {round(predict-1000)} SR  To  {round(predict+1000)} SR")
        st.markdown("***")
        st.subheader(f"Predicted price for this Computer could be between {round((predict-1000)/3.76)} USD  To  {round((predict+1000)/3.76)} USA")
...
st.caption("By: Waleed Hassan - Epsilon Data Scince Project - 19-12-2022")